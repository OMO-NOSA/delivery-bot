...
def create_pipeline(config: str = typer.Argument(..., help="Path to JSON pipeline file"),
    ...
    try:
        ...
        console.print(f"[red]Error:[/red] Invalid JSON in configuration file '{config}'")
        raise typer.Exit(1)
...
@app.command("list")
def list_pipelines(base: Optional[str] = typer.Option(None, "--base", "-b", help="Base API URL")):
...
@app.command("get")
def get_pipeline(pipeline_id: str = typer.Argument(..., help="Pipeline ID to retrieve"),
                 base: Optional[str] = typer.Option(None, "--base", "-b", help="Base API URL")):
...
@app.command("delete")
def delete_pipeline(pipeline_id: str = typer.Argument(..., help="Pipeline ID to delete"),
                    base: Optional[str] = typer.Option(None, "--base", "-b", help="Base API URL")):
...
@app.command("update")
def update_pipeline(pipeline_id: str = typer.Argument(..., help="Pipeline ID to update"),
...
@app.command("trigger")
def trigger(pipeline_id: str = typer.Argument(..., help="Pipeline ID to trigger"),
            base: Optional[str] = typer.Option(None, "--base", "-b", help="Base API URL")):
    ...
    try:
        ...
        console.print(f"[blue]Tip:[/blue] Use 'python -m cli.cli watch {out['run_id']}' to monitor execution")
    except requests.RequestException as e:
        console.print(f"[red]API Error:[/red] {e}")
        raise typer.Exit(1)

@app.command("watch")
def watch(run_id: str = typer.Argument(..., help="Run ID to watch"),
          base: Optional[str] = typer.Option(None, "--base", "-b", help="Base API URL")):
    ...
    try:
        ...
        if status == "succeeded":
            ...
            console.print(f"[yellow]Run was cancelled[/yellow]")
        else:
...
@app.command("status")
def status(base: Optional[str] = typer.Option(None, "--base", "-b", help="Base API URL")):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            console.print(f"[yellow]API responded with status: {r.status_code}[/yellow]")
    except requests.RequestException as e:
