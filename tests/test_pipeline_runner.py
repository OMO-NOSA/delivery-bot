...
from api.pipeline_runner import simulate_step, run_pipeline
...
class TestSimulateStep:
    ...
    async def test_simulate_build_step(self):
        ...
        with patch('api.pipeline_runner.db', self.db):
            with patch('api.pipeline_runner.logger') as mock_logger:
        ...
        assert "Building Docker image from Dockerfile.prod and pushing to myapp/backend" in logs
        assert "Image built and pushed successfully" in logs
        
        # Verify logger was called
        mock_logger.info.assert_called_with('build_step_complete')
    ...
    async def test_simulate_step_unknown_type(self):
        ...
        class MockStep:
            def __init__(self):
                self.name = "invalid"
                self.type = type('MockStepType', (), {'value': 'unknown'})()  # Mock enum-like object
    ...
    async def test_simulate_step_timing(self):
        ...
        start = time.time()
        build_step = Step(name="build", type=StepType.build, dockerfile="Dockerfile", ecr_repo="repo")
...
class TestRunPipeline:
    ...
    def setup_method(self):
        ...
        self.pipeline = Pipeline(
            ...
            steps=[
                ...
                Step(name="build", type=StepType.build, dockerfile="Dockerfile", ecr_repo="app/backend"),
    ...
    async def test_run_pipeline_with_failure(self):
        ...
        # Mock simulate_step to raise an exception
        with patch('api.pipeline_runner.db', self.db):
            with patch('api.pipeline_runner.simulate_step', side_effect=Exception("Step failed")):
                await run_pipeline(self.pipeline, self.run)
    ...
    async def test_run_pipeline_empty_steps(self):
        ...
        # Should succeed with empty steps
        updated_run = self.db.get_run(self.run.id)
        assert updated_run.status == RunStatus.succeeded
        assert updated_run.started_at is not None
        assert updated_run.finished_at is not None
    ...
    async def test_run_pipeline_concurrent_execution(self):
        ...
        with patch('api.pipeline_runner.db', self.db):
            ...
            await asyncio.gather(
                run_pipeline(self.pipeline, run1),
                run_pipeline(self.pipeline, run2)
            )
    ...
    async def test_run_pipeline_step_order(self):
        ...
        async def track_step_order(run, step, index):
            ...
            run.logs.append(f"[step {index+1}] Starting '{step.name}' of type '{step.type.value}'")
            self.db.update_run(run.id, run)
        
        with patch('api.pipeline_runner.db', self.db):
            with patch('api.pipeline_runner.simulate_step', side_effect=track_step_order):
                await run_pipeline(self.pipeline, self.run)
    ...
    async def test_run_pipeline_exception_in_middle_step(self):
        ...
        with patch('api.pipeline_runner.db', self.db):
            with patch('api.pipeline_runner.simulate_step', side_effect=fail_on_second_step):
                await run_pipeline(self.pipeline, self.run)
