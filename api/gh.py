...
def generate_workflow_steps(pipeline_steps: list) -> str:
    ...
    for i, step in enumerate(pipeline_steps, 1):
        ...
        if step_type == 'run':
            ...
            if command:
                workflow_steps.append(f"""    - name: {step_name}
      run: |
...
def create_branch_from_ref(
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"Failed to get SHA for base ref '{base_ref}'. Status: {response.status_code}")
        ...
        if response.status_code == 201:
            ...
            print(f"Failed to create branch '{new_branch}'. Status: {response.status_code}")
            print(f"Response: {response.text}")
            return False
...
def workflow_exists(
    ...
    if response.status_code == 200:
        ...
        print(f"Workflow {workflow_name} not found in main branch. Status: {response.status_code}")
        return False
...
def ensure_pipeline_workflow(
    ...
    try:
        ...
        status_code = create_pipeline_workflow(owner, repo, workflow_name, ref, token, pipeline_id, pipeline_steps)
...
def create_and_merge_workflow_pr(
    ...
    """
    print(f"Creating and auto-merging workflow for pipeline {pipeline_id} in {owner}/{repo}")
    ...
    branch_created = create_branch_from_ref(owner, repo, ref, branch_name, token)
    if not branch_created:
        print(f"Failed to create branch {branch_name} from {ref}, trying main branch...")
    ...
    print(f"Generating workflow steps for {len(pipeline_steps) if pipeline_steps else 0} steps")
    try:
        workflow_steps = generate_workflow_steps(pipeline_steps)
...
def auto_merge_pull_request(
    ...
    try:
        ...
        response = requests.put(merge_url, headers=headers, json=merge_payload, timeout=15)
        
        if response.status_code == 200:
            print(f"PR #{pr_number} merged successfully!")
            return True
        else:
            print(f"Failed to merge PR #{pr_number}. Status: {response.status_code}, Response: {response.text}")
            return False
