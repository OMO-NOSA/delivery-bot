...
class PipelineExecutor:
    ...
    def __init__(self, logger: Optional[LoggerInterface] = None, storage: Optional[Any] = None):
        """
    ...
    def validate_pipeline(self, pipeline: Pipeline) -> None:
        ...
        for i, step in enumerate(pipeline.steps):
            ...
            if step.type == StepType.build and not step.dockerfile:
                self.logger.warning(f"Build step {i} has no dockerfile specified, using default")
            if step.type == StepType.deploy and not step.manifest:
                self.logger.warning(f"Deploy step {i} has no manifest specified, using default")
